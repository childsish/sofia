...
class Distributor2(object):
	def __init__(self, max_jobs, sleep=30.0):
		self.__jobs = []
		self.__sleep = sleep
...
class Distributor:
	...
	def distribute(self, cmdgen, n_jobs=None, label=None):
		...
		#  A dirty hack just to get things working for now. I need a way to
		# map the filename to the arguments. Otherwise I'm trying to write
		# filenames that are too long.
		outfile = open(os.path.join(cluster.jobdir, label,
		 '%s.map'%(timestamp,)), 'w')
		if n_jobs == None:
			tmpdir = None
			# Create all the jobs.
			...
			for args, arglist in cmdgen.generateArguments():
				jobs.append(CpuJob(args, t_jobdir, t_logdir, str(c_job)))
